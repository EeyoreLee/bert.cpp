...
std::vector<std::vector<int>> bert_tokenizer::batch_encode(const std::vector<std::string> &batch_text)
{
    std::vector<std::vector<int>> batch_input_ids;
    int max_length = 0;
    for (auto text : batch_text)
    {
        std::vector<int> input_ids = this->encode(text);
        max_length = (input_ids.size() > max_length) ? input_ids.size() : max_length;
        batch_input_ids.push_back(input_ids);
    }
    for (auto &ids : batch_input_ids)
    {
